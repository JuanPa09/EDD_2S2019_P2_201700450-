...
class AVLTree:
	...
	def insertarArbol(self):
		tdatos=len(tree.altura)
		nmax=0
		for i in range(tdatos):
			if tree.altura[i]>nmax:
				nmax=tree.altura[i]
			pass
		#print(nmax)
		...
		while nmax!=0:
			for i in range(tdatos):
				if tree.altura[i]==nmax:
					print("Carne: "+str(tree.carne[i])+", Nombre: "+str(tree.nombre[i])+", Altura: "+str(tree.altura[i]))#Aca se insertaria al arbolfe
			nmax=nmax-1
...
tree.insert(8,"Lopez")
tree.insert(9,"Hola")
tree.insert(10,"Mundo")
tree.print_tree()#Funcion que mete los datos al vector
tree.insertarArbol()




#Buscar Numero Mayor para que sea el root
